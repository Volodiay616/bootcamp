...
class TestOrderLineTag(TransactionCase):
    ...
    def test_so_line_tag_access(self):
        ...
        LineTagSeniorSalesman = self.env["sale.order.line.tag"].with_user(self.user_senior_salesman)
        ...
        tag_senior_salesman = LineTagSeniorSalesman.create({"name": "Tag Senior Salesman"})
        tag_manager = LineTagManager.create({"name": "Tag Manager"})
                
        # Fetch tags as user Salesman
        tag_ids = LineTagSalesman.search([("id", "in", [tag_salesman.id, tag_manager.id, tag_senior_salesman.id])])
        ...
        tag_senior_salesman_ids = LineTagSeniorSalesman.search([("id", "in", [tag_salesman.id, tag_manager.id, tag_senior_salesman.id])])
        ...
        tag_manager_ids = LineTagManager.search([("id", "in", [tag_salesman.id, tag_manager.id, tag_senior_salesman.id])])
